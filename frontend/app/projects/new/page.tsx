...
import { saveProjectTitle } from "@/lib/project-titles";
...
export default function NewProjectPage() {
  ...
  const { execute: deployProject, isLoading, isSuccess, receiptData } = useCreateProject();

  useEffect(() => {
    if (isSuccess && receiptData) {
      if (projectTitle.trim()) {
        const logs = parseEventLogs({ abi: FACTORY_ABI, eventName: "ProjectCreated", logs: receiptData.logs });
        const addr = logs[0]?.args?.projectAddress;
        if (addr) saveProjectTitle(addr, projectTitle.trim());
      }
      router.push("/dashboard");
    }
  }, [isSuccess, receiptData, router, projectTitle]);
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>A short name to identify this project in your dashboard.</CardDescription>
        </CardHeader>
        <CardContent>
          <Input
            id="projectTitle"
